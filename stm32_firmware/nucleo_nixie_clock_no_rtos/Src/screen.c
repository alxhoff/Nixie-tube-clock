...
screen_device_t screen_dev = { .cursor_period = SCREEN_CURSOR_PERIOD,
	.draw_text = &SCREEN_DRAW, .clear_screen = &SCREEN_CLEAR, .update_screen =
			&SCREEN_REFRESH,
#ifdef SCREEN_USE_CURSOR
...
if (screen_dev.framebuffer
	...
	if (screen_dev.framebuffer[screen_dev.cursor_location_y]) {
		int max_len = strlen(
				screen_dev.framebuffer[screen_dev.cursor_location_y]);
...
for (unsigned char i = 0; i < screen_dev.row_count; i++)
	memset(screen_dev.framebuffer[i], 0, sizeof(char) * (screen_dev.cols + 1));
}
...
signed char screen_add_line(char *line) {
	...
	screen_dev.framebuffer[screen_dev.row_count] = malloc(
...
signed char screen_add_line_at_index(unsigned char index, char *line) {
	...
	screen_dev.framebuffer[index] = realloc(screen_dev.framebuffer[index],
			sizeof(char) * (strlen(line) + 1));
