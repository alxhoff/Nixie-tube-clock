...
signed char DS3231_write_time(I2C_HandleTypeDef *hi2c, ds3231_time_t* time) {
	...
	if(HAL_I2C_Mem_Write(hi2c, DS3231_ADDR8, 0x00, 1, write_buffer, 7, 10) != HAL_OK)
		return -1;
...
if (HAL_I2C_Mem_Read(hi2c, DS3231_ADDR8, 0x11, 1, read_buffer, 2, 10) != HAL_OK)
	return -1;

read_buffer[1] = (read_buffer[1] >> 6) * 25;

*temp = ((((short) read_buffer[0] << 8) | (short) read_buffer[1]) >> 6) / 4.0f;
...
if (HAL_I2C_Mem_Write(hi2c, DS3231_ADDR8, alarm_register_addr, 1, write_buffer,
		4, 10) != HAL_OK)
	return -1;
...
HAL_I2C_Mem_Read(hi2c, DS3231_ADDR8, alarm_register_addr, 1, read_buffer, 4,
		10);
