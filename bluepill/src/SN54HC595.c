...
void output_self(shift_array_t *self, unsigned char byte_count)
{
	...
	HAL_GPIO_WritePin(self->ser_clk_port, self->ser_clk_pin,
			  GPIO_PIN_RESET);
	for (unsigned char i = byte_count; i > 0; i--) {
		for (unsigned char j = 8; j > 0; j--) {
			if (*(self->out_buf +
			    ...
			    (1 << (j - 1)))
				HAL_GPIO_WritePin(self->ser_in_port,
						  self->ser_in_pin,
						  GPIO_PIN_SET);
			else
				HAL_GPIO_WritePin(self->ser_in_port,
		       ...
		       uint32_t delay)
{
	...
	for (unsigned char i = 0; i < byte_count; i++) {
		for (unsigned char j = 0; j < 8; j++) {
			if (*(self->out_buf + (i * sizeof(unsigned char))) &
			    (1 << j))
				HAL_GPIO_WritePin(self->ser_in_port,
						  self->ser_in_pin,
						  GPIO_PIN_SET);
...
{
	HAL_GPIO_WritePin(self->out_ena_port, self->out_ena_pin,
			  GPIO_PIN_RESET);
}
...
static void SN54HC595_init_obj(shift_array_t *shift)
{
	...
	shift->out_buf = (unsigned char *)calloc(1, sizeof(unsigned char) *
							    shift->dev_count);
